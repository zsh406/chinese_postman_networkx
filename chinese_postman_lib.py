...
def make_graph():
  ...
  g.add_weighted_edges_from([('a','b',1),('a','c',1),('a','d',1),('a','e',1)])
  # networkx.draw(g)
  # plt.show()
  return g
...
def optimize_dead_ends(in_g, out_g):
  ...
  for node in dead_ends(in_g):
    ...
    new_to = next(n for n in networkx.dfs_preorder_nodes(
        in_g, node) if in_g.degree(n) > 2)
    print("DEBUG: I will optimize by adding an edge from %s to %s" % (
        in_g.node[new_from], in_g.node[new_to]))
...
def add_artificial_edge(in_g, out_g, source, target):
  new_path = networkx.dijkstra_path(in_g, source=source, target=target, weight='weight')
  # new_path_streets = []
  new_path_length = 0
  for i in range(1, len(new_path)):
    hop_source = new_path[i - 1]
    hop_target = new_path[i]
    new_path_length += in_g[hop_source][hop_target]['weight']
  ...
  out_g.add_edge(source, target,weight = new_path_length)
...
def cardinal_direction(g, source, dest):
  ...
  dx = lon2 - lon1
  dy = lat2 - lat1
  angle_pis = old_div(math.atan2(dy, dx), math.pi)
  if angle_pis >= (old_div(-1.0,8))  and angle_pis < (old_div(1.0,8)):
    return "east"
  if angle_pis >= (old_div(1.0,8)) and angle_pis < (old_div(3.0,8)):
    return "northeast"
  if angle_pis >= (old_div(3.0,8)) and angle_pis < (old_div(5.0,8)):
    return "north"
  if angle_pis >= (old_div(5.0, 8)) and angle_pis < (old_div(7.0,8)):
    return "northwest"
  if angle_pis >= (old_div(7.0,8)) or angle_pis < (old_div(-7.0,8)):
    return "west"
  if angle_pis >= (old_div(-7.0,8)) and angle_pis < (old_div(-5.0,8)):
    return "southwest"
  if angle_pis >= (old_div(-5.0,8)) and angle_pis < (old_div(-3.0,8)):
    return "south"
  if angle_pis >= (old_div(-3.0,8)) and angle_pis < (old_div(-1.0,8)):
    return "southeast"
